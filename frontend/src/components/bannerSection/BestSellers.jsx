...
const ProductSkeleton = ({ rank }) => {
  return (
    <div className="relative bg-white rounded-xl overflow-hidden border border-gray-100 shadow-sm h-full flex flex-col">
      {/* Rank Badge Skeleton */}
      {rank <= 2 && (
        <div className={`absolute -top-2 -left-2 w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shadow-md z-10 ${
          rank === 0 ? 'bg-yellow-200' : rank === 1 ? 'bg-gray-200' : 'bg-orange-200'
        }`}>
...
const BestSellers = () => {
  ...
  const rankBadge = (index) => {
    const badges = [
      { bg: 'bg-yellow-400', text: 'text-yellow-900', icon: FaTrophy },
      { bg: 'bg-gray-300', text: 'text-gray-800', num: '2' },
      { bg: 'bg-orange-400', text: 'text-white', num: '3' }
    ];
    ...
    return (
      <div className={`absolute -top-2 -left-2 w-8 h-8 ${badge.bg} ${badge.text} rounded-full flex items-center justify-center text-xs font-bold shadow-md z-10`}>
        {badge.icon ? <badge.icon size={14} /> : badge.num}
      </div>
    );
  };
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="flex flex-col items-center mb-10">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-orange-600 font-bold text-sm uppercase tracking-wider">Best Sellers</span>
            <FaFire className="text-orange-500 text-lg" />
          </div>
          ...
          <>
            ...
            <div className="flex justify-center mt-10">
              <Link to="/shop?sort=bestselling">
                <button className="flex items-center gap-2 bg-orange-500 text-white px-8 py-3 rounded-lg font-bold hover:bg-orange-600 transition-colors shadow-md hover:shadow-lg transform hover:-translate-y-0.5 ">
                  View All Best Sellers <FaLongArrowAltRight />
                </button>
              </Link>
            </div>
          </>
