...
const Order = () => {
  ...
  const discount = order?.orderItems.reduce(
    (acc, item) => acc + (item.discountPercentage / 100) * item.qty * item.price,
    0
  );
      ...
      <div className="container mx-auto px-4 mt-[100px]">
        ...
        >
          ...
          <div className="mt-6 inline-flex items-center gap-2 bg-white px-6 py-2 rounded-full border border-gray-200 shadow-sm">
            ...
            <span className="text-blue-600 font-mono font-black">{order.orderId}</span>
          </div>
        </motion.div>
            ...
            <div className="bg-white rounded-[2.5rem] border border-gray-100 overflow-hidden shadow-sm">
                    ...
                    {order.orderItems.map((item, index) => (
                      <tr key={index} className="hover:bg-gray-50/30 transition-colors">
                        <td className="px-8 py-6">
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              ...
                              <p className="text-[10px] text-gray-400 font-mono mt-1">UNIT PRICE: ৳{item.price}</p>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="px-8 py-6 text-right font-mono font-black text-gray-900">
                          ৳{(item.qty * (item.price - (item.discountPercentage / 100) * item.price)).toFixed(2)}
                        </td>
                      </tr>
                    ))}
                  </tbody>
              ...
              <div className="p-8 bg-gray-50/50 border-t border-gray-100">
                <div className="space-y-3 max-w-xs ml-auto">
                  ...
                  <div className="flex justify-between text-sm font-mono text-gray-500">
                    <span>SHIPPING</span>
                    <span className="text-gray-900 font-bold">৳{shippingCharge.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between items-center pt-4 border-t border-gray-200">
                    <span className="text-lg font-mono font-black text-gray-900 uppercase">TOTAL</span>
                    <span className="text-2xl font-mono font-black text-blue-600">৳{totalPrice.toFixed(2)}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="lg:w-1/3 space-y-6">
            ...
            <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm">
              <div className="flex items-center gap-4 mb-8">
                ...
                <h3 className="text-lg font-mono font-black uppercase tracking-tighter">Order Status</h3>
              </div>
              <div className="space-y-6 font-mono">
                <div className="flex justify-between items-center p-4 bg-gray-50 rounded-2xl">
                  <span className="text-[11px] font-black text-gray-400 uppercase">Payment</span>
                ...
                <div className="flex justify-between items-center p-4 bg-gray-50 rounded-2xl">
                  <span className="text-[11px] font-black text-gray-400 uppercase">Delivery</span>
                ...
                <div className="pt-4 flex items-center justify-between text-gray-400 italic text-[10px]">
            ...
            {userInfo?.isAdmin && (
              <div className="flex flex-col gap-3 pt-4">
                ...
                {!order.isDelivered && (
                   <button className="w-full flex items-center justify-center gap-3 bg-blue-600 text-white py-4 rounded-2xl font-mono font-black uppercase tracking-widest text-xs hover:bg-green-600 transition-all">
