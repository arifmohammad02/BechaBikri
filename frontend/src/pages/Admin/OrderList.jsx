          ...
          >
            ...
            {!isDashboard && (
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6 bg-white p-5 border border-gray-100 shadow-sm">
                ...
                <input
                  type="date"
                  className="p-3 bg-gray-50 border border-gray-200 text-xs font-bold"
                  value={startDate}
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-xs">
                  <thead className="bg-black text-gray-400 uppercase tracking-widest">
                    <tr>
                      <th className="px-4 py-4 font-medium border-r border-gray-800">
                        Items
                      </th>
                      <th className="px-4 py-4 font-medium border-r border-gray-800">
                        ID
                      ...
                      >
                        <td className="px-4 py-4">
                          <img
                            ...
                            className="w-12 h-12 object-cover grayscale group-hover:grayscale-0 transition-all border border-gray-200 rounded"
                          />
                        </td>
                        <td className="px-4 py-4 font-bold text-gray-900">
                          {order.orderId}
                        </td>
                        <td className="px-4 py-4 uppercase text-gray-600">
                        ...
                        <td className="px-4 py-4">
                          <span
                            className={`px-2 py-1 rounded-[4px] text-[9px] font-black uppercase ${
                        ...
                        <td className="px-4 py-4 text-center">
                          {needsVerification(order) ? (
                            <button
                              ...
                              className="bg-amber-500 text-white px-4 py-2 text-[10px] tracking-widest hover:bg-amber-600 transition-all uppercase font-bold rounded animate-pulse flex items-center justify-center gap-2"
                            >
                              <FaEye /> Verify
                            </button>
                          ) : (
            ...
            >
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-2xl font-mono font-black uppercase tracking-tighter">
                  Verify <span className="text-amber-600">Payment</span>
                </h3>
                <button 
              ...
              <div className="space-y-4 mb-6">
                ...
                <div className="bg-gray-50 p-4 rounded-2xl">
                  <p className="text-xs text-gray-500 uppercase font-mono mb-1">Order ID</p>
                  <p className="font-bold font-mono text-lg">{verifyingOrder.orderId}</p>
                </div>

                {/* Payment Method */}
                <div className="bg-gray-50 p-4 rounded-2xl">
                  <p className="text-xs text-gray-500 uppercase font-mono mb-1">Payment Method</p>
                  <p className="font-bold font-mono text-lg">{verifyingOrder.paymentMethod}</p>
                </div>

                {/* Transaction ID */}
                <div className="bg-blue-50 p-4 rounded-2xl border-2 border-blue-100">
                  <p className="text-xs text-blue-600 uppercase font-mono mb-1">Transaction ID</p>
                  <p className="font-bold font-mono text-2xl tracking-wider text-blue-900">
                    {verifyingOrder.manualPaymentDetails?.transactionId || "N/A"}
                  </p>
                </div>

                {/* Sender Number */}
                <div className="bg-gray-50 p-4 rounded-2xl">
                  <p className="text-xs text-gray-500 uppercase font-mono mb-1">Sent From</p>
                  <p className="font-bold font-mono text-lg">{verifyingOrder.manualPaymentDetails?.senderNumber || "N/A"}</p>
                </div>

                {/* Amount */}
                <div className="bg-gray-50 p-4 rounded-2xl">
                  <p className="text-xs text-gray-500 uppercase font-mono mb-1">Amount</p>
                  <p className="font-bold font-mono text-xl text-red-600">৳{verifyingOrder.totalPrice}</p>
                </div>

                {/* Screenshot Preview */}
                {verifyingOrder.manualPaymentDetails?.paymentScreenshot && (
                  <div className="bg-gray-50 p-4 rounded-2xl">
                    <p className="text-xs text-gray-500 uppercase font-mono mb-2">Screenshot</p>
                    <img 
                      src={verifyingOrder.manualPaymentDetails.paymentScreenshot} 
                      alt="Payment Proof" 
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full p-4 bg-gray-50 border border-gray-200 rounded-2xl font-mono text-sm resize-none outline-none focus:ring-2 focus:ring-blue-500"
                    rows={3}
                  />
                </div>
              </div>
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 py-4 bg-emerald-600 text-white rounded-2xl font-mono font-black uppercase tracking-widest hover:bg-emerald-700 transition-all flex items-center justify-center gap-2"
                >
                  <FaCheck /> Approve
                </button>
                <button
                  onClick={() => handleVerifyPayment("failed")}
                  className="flex-1 py-4 bg-red-600 text-white rounded-2xl font-mono font-black uppercase tracking-widest hover:bg-red-700 transition-all flex items-center justify-center gap-2"
                >
                  <FaTimes /> Reject
                </button>
              </div>
